 ...
 *   https://datatables.net/download/#bs4/dt-1.10.24/e-2.0.0/b-1.7.0/date-1.0.2/fc-3.3.2/sl-1.3.2
 *
 * Included libraries:
 *   DataTables 1.10.24, Editor 2.0.0, Buttons 1.7.0, DateTime 1.0.2, FixedColumns 3.3.2, Select 1.3.2
 */
...
(function( $, window, document, undefined ) {
	...
	DataTable.ext = _ext = {
		 ...
		 *  @type string
		 */
		build:"bs4/dt-1.10.24/e-2.0.0/b-1.7.0/date-1.0.2/fc-3.3.2/sl-1.3.2",
...
var DataTable = $.fn.dataTable;
var DtInternalApi = DataTable.ext.oApi;
...
var dataSource = {
    ...
    fakeRow: function (insertPoint) {
        ...
        for (var i = 0, ien = dt.columns(':visible').count(); i < ien; i++) {
            ...
            var fields = __dtFieldsFromIdx(dt, this.s.fields, i, true);
            var cell = dt.cell(':eq(0)', i + ':visible').node();
            if (cell) {
    ...
    edit: function (identifier, fields, data, store) {
        ...
        if (!__dtIsSsp(dt, this) || this.s.editOpts.drawType === 'none') {
            ...
            var rowId = dataSource.id.call(this, data);
            var row;
            ...
            if (!row.any()) {
                row = dt.row(function (rowIdx, rowData, rowNode) {
                    return rowId == dataSource.id.call(that, rowData);
                });
            }
            if (row.any()) {
    ...
    remove: function (identifier, fields, store) {
        ...
        if (cancelled.length === 0) {
            ...
            dt.rows(identifier).every(function () {
                var id = dataSource.id.call(that, this.data());
    ...
    prep: function (action, identifier, submit, json, store) {
        ...
        if (action === 'create') {
            store.rowIds = $.map(json.data, function (row) {
                return dataSource.id.call(_this, row);
            });
        }
        if (action === 'edit') {
        ...
        if (!__dtIsSsp(dt, this) && action === 'edit' && store.rowIds.length) {
            ...
            var compare = function (id) {
                return function (rowIdx, rowData, rowNode) {
                    return id == dataSource.id.call(that, rowData);
                };
            };
...
var dataSource$1 = {
    ...
    individual: function (identifier, fieldNames) {
        ...
        var out = dataSource$1.fields.call(this, identifier);
        var fields = this.s.fields;
        var forceFields = {};
        $.each(fieldNames, function (i, name) {
            forceFields[name] = fields[name];
    ...
    fields: function (identifier) {
        ...
        if (Array.isArray(identifier)) {
            for (var i = 0, ien = identifier.length; i < ien; i++) {
                var res = dataSource$1.fields.call(this, identifier[i]);
    ...
    create: function (fields, data) {
        ...
        if (data) {
            var id = dataSource$1.id.call(this, data);
            try {
    ...
    edit: function (identifier, fields, data) {
        ...
        var id = dataSource$1.id.call(this, data) || 'keyless';
        __html_set(id, fields, data);
    },
...
function heightCalc(dte) {
    // Set the max-height for the form content
    var header = $('div.DTE_Header', dom.wrapper).outerHeight();
    var footer = $('div.DTE_Footer', dom.wrapper).outerHeight();
...
function hide(dte, callback) {
    ...
    if (displayed) {
        $(dom.content).animate({
            top: -(dom.content.offsetHeight + 50)
        }, 600, function () {
            $([dom.wrapper, dom.background]).fadeOut('normal', function () {
                $(this).detach();
                callback();
            });
        });
...
function init() {
    dom.content = $('div.DTED_Envelope_Container', dom.wrapper)[0];
    cssBackgroundOpacity = $(dom.background).css('opacity');
}
function show(dte, callback) {
    ...
    if (!displayed) {
        var style = dom.wrapper.style;
        style.opacity = '0';
        style.display = 'block';
        var height = heightCalc(dte);
        var targetRow = findAttachRow(dte, envelope.conf.attach);
        var width = targetRow.offsetWidth;
        style.display = 'none';
        style.opacity = '1';
        // Prep the display
        dom.wrapper.style.width = width + "px";
        dom.wrapper.style.marginLeft = -(width / 2) + "px";
        dom.wrapper.style.top = ($(targetRow).offset().top + targetRow.offsetHeight) + "px";
        dom.content.style.top = ((-1 * height) - 20) + "px";
        // Start animating in the background
        dom.background.style.opacity = '0';
        dom.background.style.display = 'block';
        $(dom.background).animate({
            opacity: cssBackgroundOpacity
        }, 'normal');
        // Animate in the display
        $(dom.wrapper).fadeIn();
        $(dom.content).animate({ top: 0 }, 600, callback);
    }
    // Event handlers
    $(dom.close)
...
var envelope = {
    ...
    open: function (dte, append, callback) {
        $(dom.content).children().detach();
        dom.content.appendChild(append);
...
function heightCalc$1() {
    var headerFooter = $('div.DTE_Header', dom$1.wrapper).outerHeight() +
        $('div.DTE_Footer', dom$1.wrapper).outerHeight();
    if (isMobile()) {
        $('div.DTE_Body_Content', dom$1.wrapper).css('maxHeight', 'calc(100vh - ' + headerFooter + 'px)');
    }
    else {
        // Set the max-height for the form content
...
function init$1() {
    ...
    dom$1.content = $('div.DTED_Lightbox_Content', dom$1.wrapper);
    dom$1.wrapper.css('opacity', 0);
    dom$1.background.css('opacity', 0);
    ready = true;
}
...
function bubblePosition() {
    ...
    var top = position.top, left = (position.left + position.right) / 2, width = liner.outerWidth(), visLeft = left - (width / 2), visRight = visLeft + width, docWidth = $(window).width(), padding = 15, classes = this.classes.bubble;
    wrapper.css({
        top: top,
        left: left
    });
...
function title(title) {
    ...
    if (typeof title === 'function') {
        title = title(this, new DataTable$1.Api(this.s.table));
    }
    header.html(title);
    return this;
}
 ...
 *     been uploaded
 */
function upload(editor, conf, files, progressCallback, completeCallback) {
...
function _message(el, msg, title, fn) {
    ...
    if (typeof msg === 'function') {
        msg = msg(this, new DataTable$2.Api(this.s.table));
    }
    el = $(el);
    if (canAnimate) {
        el.stop();
...
var upload$1 = $.extend(true, {}, baseFieldType, {
    create: function (conf) {
        var editor = this;
        var container = _commonUpload(editor, conf, function (val) {
            upload$1.set.call(editor, conf, val[0]);
...
var datatable = $.extend(true, {}, baseFieldType, {
    ...
    create: function (conf) {
        ...
        var container = $(dt.table(undefined).container());
        return {
            input: container,
            side: $('<div class="DTE_Field_Type_datatable_info">')
    ...
    set: function (conf, val, localUpdate) {
        ...
        // 	conf._lastSet = val;
        // }
        var valueProp = conf.optionsPair.value;
...
var Field = /** @class */ (function () {
    ...
    Field.prototype._msg = function (el, msg, fn) {
        ...
        if (typeof msg === 'function') {
            ...
            msg = msg(editor, new DataTable$4.Api(editor.internalSettings().table));
        }
 ...
 */
var apiRegister = DataTable$5.Api.register;
...
$.extend(_buttons, {
    ...
    createInline: {
        ...
        formOptions: {},
        action: function (e, dt, node, config) {
            config.editor.inlineCreate(config.formOptions);
        }
    },
    edit: {
        extend: 'selected',
...
var Editor = /** @class */ (function () {
    ...
    // These are not publically documented.
    /** @internal */
